...
object Day5AMazeOfTwistyTrampolines {

    fun countStepsToReachTheExit(input: String, decrease: Boolean): Int {
        val instructions = input.split("\n").map { it.toInt() }
        return countSteps(0, instructions, 0, decrease)
    }

    private tailrec fun countSteps(steps: Int, instructions: List<Int>, position: Int, decrease: Boolean): Int {
        if (position < 0 || position >= instructions.size) {
            return steps
        }
        val offset = instructions[position]
        val newInstructions = instructions.mapIndexed { index, o ->
            if (index == position) calculateNewOffset(o, decrease) else o
        }
        return countSteps(steps + 1, newInstructions, position + offset, decrease)
    }

    private fun calculateNewOffset(offset: Int, decrease: Boolean) = if (decrease && offset >= 3) offset - 1 else offset + 1

}

private val inputFileName = "day5.txt"

fun main(args: Array<String>) {
    val input = FileUtil.readFile(inputFileName)
    println("Steps one: " + Day5AMazeOfTwistyTrampolines.countStepsToReachTheExit(input, false))
    println("Steps two: " + Day5AMazeOfTwistyTrampolines.countStepsToReachTheExit(input, true))
}
