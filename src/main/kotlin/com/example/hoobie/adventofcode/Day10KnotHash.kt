...
and it also ensures that your input lengths are never larger than 255. For example, if you are given 1,2,3,
...
For example, if the previous example was your first round, you would start your second round
with the same length sequence (3, 4, 1, 5, 17, 31, 73, 47, 23, now assuming they came from ASCII codes
...
object Day10KnotHash {
    ...
    fun knotHash(input: String): String {
        val asciiCodes = input.toCharArray().map { it.toInt() }.joinToString(separator = ",")
        val extraSeparator = if (input.isNotEmpty()) "," else ""
        val standardLengths = "17,31,73,47,23"
        val newInput = asciiCodes + extraSeparator + standardLengths
        val lengths = newInput.split(",").map { it.toInt() }

        var list = IntCircularList()
        var current = 0
        var skip = 0
        for (i in 0..63) {
            val results = hashAndMultiplyTheFirstTwoNumbers(newInput, 0, 255, current = current, skip = skip,
                    lengths = lengths, list = list)
            list = results.first
            current = results.second.first
            skip = results.second.second
        }

        return (list as ArrayList<Int>)
                .chunked(16)
                .map { it.reduce { acc, i -> acc.xor(i) } }
                .map { it.toString(16) }.joinToString(separator = "") { it.padStart(2, '0') }
    }

    fun hashAndMultiplyTheFirstTwoNumbers(input: String, start: Int, end: Int, current: Int = 0, skip: Int = 0,
                                          list: IntCircularList = IntCircularList(), lengths: List<Int>? = null)
            : Pair<IntCircularList, Pair<Int, Int>> {

        if (list.isEmpty()) {
            list += start..end
        }

        var currCopy = current
        var skipCopy = skip
        val lengthsCopy = lengths ?: input.split(",").map { it.toInt() }

        for (length in lengthsCopy) {
            currCopy = hash(list, length, currCopy, skipCopy)
            skipCopy++
        }

        return Pair(list, Pair(currCopy, skipCopy))
    }
...
fun main(args: Array<String>) {
    println("Knot hash: " + Day10KnotHash.knotHash(input))
}
