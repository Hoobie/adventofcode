...
object Day19ASeriesOfTubes {

    fun getLettersAndSteps(input: String): Pair<String, Long> {
        val diagram = input.split("\n").map { it.toCharArray().toList() }
        val x = diagram[0].indexOf('|')
        return move("", 1, 0, x, 0, Pair(0, 1), diagram)
    }

    private tailrec fun move(letters: String, counter: Long, lastLetterStep: Long, x: Int, y: Int, vector: Pair<Int, Int>, diagram: List<List<Char>>): Pair<String, Long> {
        val current = diagram[y][x]
        if (y + vector.second < 0 || y + vector.second >= diagram.size
                || x + vector.first < 0 || x + vector.first >= diagram[y].size)
            return Pair(letters + current, lastLetterStep)

        if (current == '+' && vector.first == 0 && diagram[y][x - 1] == '-')
            return move(letters, counter + 1, lastLetterStep, x - 1, y, Pair(-1, 0), diagram)
        if (current == '+' && vector.first == 0 && diagram[y][x + 1] == '-')
            return move(letters, counter + 1, lastLetterStep, x + 1, y, Pair(1, 0), diagram)
        if (current == '+' && vector.second == 0 && diagram[y - 1][x] == '|')
            return move(letters, counter + 1, lastLetterStep, x, y - 1, Pair(0, -1), diagram)
        if (current == '+' && vector.second == 0 && diagram[y + 1][x] == '|')
            return move(letters, counter + 1, lastLetterStep, x, y + 1, Pair(0, 1), diagram)

        if (current.isLetter())
            return move(letters + current, counter + 1, counter, x + vector.first, y + vector.second, vector, diagram)
        return move(letters, counter + 1, lastLetterStep, x + vector.first, y + vector.second, vector, diagram)
    }

}

private val inputFileName = "day19.txt"

fun main(args: Array<String>) {
    val input = FileUtil.readFile(inputFileName)

    println("Letters and steps: " + Day19ASeriesOfTubes.getLettersAndSteps(input))
}
