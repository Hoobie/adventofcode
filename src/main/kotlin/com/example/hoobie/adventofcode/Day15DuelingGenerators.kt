...
only occasionally handing an acceptable value to the judge, and otherwise working through the same sequence of values 
as before until they find one.
...
object Day15DuelingGenerators {

    fun getFinalCount(input: String, pairsCount: Int, generator1: (Long, Long) -> Long, generator2: (Long, Long) -> Long): Int {
        val generators = input
                .split("\n")
                .map { it.split(" ").last().toLong() }

        return (0..pairsCount).fold(Pair(generators, 0), { acc, _ ->
            val a = generator1(acc.first[0], 16807)
            val b = generator2(acc.first[1], 48271)
            Pair(listOf(a, b), acc.second + if (a.to16Bits() == b.to16Bits()) 1 else 0)
        }).second
    }

    private fun Long.to16Bits() = this.toString(2).takeLast(16)
}

private val input = "Generator A starts with 679\nGenerator B starts with 771"

// Long extensions
private tailrec fun Long.nextDivisible(factor: Long, divider: Long): Long {
    val new = ((this * factor) % 2147483647)
    return if ((new % divider).compareTo(0) == 0) new else new.nextDivisible(factor, divider)
}

private fun Long.next(factor: Long) = (this * factor) % 2147483647

fun main(args: Array<String>) {
    val generator1: (Long, Long) -> Long = { number, factor -> number.next(factor) }
    println("Final count part 1: " + Day15DuelingGenerators.getFinalCount(input, 40000000, generator1, generator1))
    println("Final count part 2: " + Day15DuelingGenerators.getFinalCount(input, 5000000,
            { number, factor -> number.nextDivisible(factor, 4) }, { number, factor -> number.nextDivisible(factor, 8) }))
}
