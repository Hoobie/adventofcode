...
as the new current position a total of 2017 times, inserting 2017 as its final operation, 
 ...
 */
object Day17Spinlock {

    fun getNext(input: String, lastValue: Int): Int {
        val steps = input.toInt()
        val list = listOf(0)

        val result = (1..lastValue).fold(Pair(0, list), { acc: Pair<Int, List<Int>>, i ->
            val nextIdx = (acc.first + 1 + steps) % acc.second.size
            val newList = acc.second.insertAfter(nextIdx, i)
            Pair(nextIdx, newList)
        })
        return result.second[result.first + 2]
    }

    fun getNext2(input: String, lastValue: Int): Int {
        val steps = input.toInt()
        val list = ImaginaryList<Int>(1) // give up on a normal list due to performance issues

        return (1..lastValue).fold(Pair(0, list), { acc: Pair<Int, ImaginaryList<Int>>, i ->
            val nextIdx = (acc.first + 1 + steps) % acc.second.size
            acc.second.insertAfter(nextIdx, i)
            Pair(nextIdx, acc.second)
        }).second.secondElement!!
    }

    private fun <T> List<T>.insertAfter(idx: Int, value: T) =
            this.take(idx + 1) + listOf(value) + this.drop(idx + 1)

    data class ImaginaryList<T>(var size: Int) {

        var secondElement: T? = null

        fun insertAfter(idx: Int, value: T) {
            if (idx == 0) secondElement = value
            size++
        }
    }
...
private val input = "343"

fun main(args: Array<String>) {
    println("Next 1: " + Day17Spinlock.getNext(input, 2017))

    println("Next 2: " + Day17Spinlock.getNext2(input, 50000000))
}
