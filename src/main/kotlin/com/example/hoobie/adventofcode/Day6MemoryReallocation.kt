...
object Day6MemoryReallocation {

    fun countCyclesAndLoopSize(input: String): Pair<Int, Int> {
        val banks = input.split("\t").map { it.toInt() }
        return countCyclesAndLoopSizeTailRec(0, banks, listOf())
    }

    private tailrec fun countCyclesAndLoopSizeTailRec(cycle: Int, banks: List<Int>, seenConfigurations: List<String>): Pair<Int, Int> {
        val conf = banks.toString()
        if (seenConfigurations.contains(conf)) {
            return Pair(cycle, seenConfigurations.size - seenConfigurations.indexOf(conf))
        }
        val newConfigurations = seenConfigurations.plusElement(conf)
        val idxAndMax = banks.foldIndexed(Pair(0, 0)) { idx, idxAndMax, blocks ->
            if (blocks > idxAndMax.second) Pair(idx, blocks) else idxAndMax
        }
        val intermediateBanks = banks.mapIndexed { idx, blocks -> if (idx == idxAndMax.first) 0 else blocks }
        val newBanks = reallocateMemory(idxAndMax.first, idxAndMax.second, intermediateBanks)
        return countCyclesAndLoopSizeTailRec(cycle + 1, newBanks, newConfigurations)
    }

    private tailrec fun reallocateMemory(startIdx: Int, blocks: Int, banks: List<Int>): List<Int> {
        if (blocks == 0) {
            return banks
        }
        val idx = if (startIdx + 1 == banks.size) 0 else startIdx + 1
        val newBanks = banks.mapIndexed { i, b -> if (i == idx) b + 1 else b }
        return reallocateMemory(idx, blocks - 1, newBanks)
    }

}

private val input = "0\t5\t10\t0\t11\t14\t13\t4\t11\t8\t8\t7\t1\t4\t12\t11"

fun main(args: Array<String>) {
    println("Cycles: " + Day6MemoryReallocation.countCyclesAndLoopSize(input))
}
